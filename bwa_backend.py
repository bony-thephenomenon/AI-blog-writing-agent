...
def _extract_text(content) -> str:
    """Normalize AIMessage.content from Gemini/OpenAI."""
...
def research_node(state: State) -> dict:
    ...
    # Fallback: if every source lacks a usable date,
    # keep the original evidence instead of returning nothing.
        evidence = dated if dated else evidence
...
def worker_node(payload: dict) -> dict:
    ...
    section_md = _extract_text(response.content).strip()
...
def _pollinations_generate_image_bytes(prompt: str, size: str = "1024x1024") -> bytes:
    """
    Generate image bytes using Pollinations.ai
    """
